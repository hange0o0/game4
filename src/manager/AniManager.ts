class AniManager {
    ...
    public preLoadMV(id){
        ...
        if(this.mcFactorys[name])
            return true;
        var data:any = MyRES.getRes(name + "_json"); //qid
        var texture:egret.Texture = MyRES.getRes(name + "_png");
        if(data && texture)
            return true;
        var groupName = 'mv' + name;
        if(!this.loadSkillGroup[groupName])
        {
            this.loadSkillGroup[groupName] = true;
            MyRES.loadGroup([name + "_json",name + "_png"]);
        }
        return false;
    }
